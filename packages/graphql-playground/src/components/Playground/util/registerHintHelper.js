   ...
   CodeMirror.registerHelper('hint', 'graphql', (editor, options) => {
     ...
     /**
      * The history hint is unique, toggling the history-options menu
      */
     results.list.unshift({
         text:'history',
         description: 'View Query History',
         hint: () => {document.getElementById('history-menu').classList.toggle('show')}
     })
     ...
     /**
      * Allow rapid access to selectedHint from editor
      */
     CodeMirror.on(results, 'select', (selectedHint) => editor.selectedHint = selectedHint)
